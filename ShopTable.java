    ...
    public String printShopItems() {
        ...
        for (ShopItem i : table) {
            if (i != null && i.weapon.getWeaponName().compareTo("DELETED") != 0) {
                s += i.weapon.getWeaponName().toUpperCase(Locale.ROOT) + " (x" + i.quantity + ") Range: "
                        + i.weapon.getRange() + " Weight: " + i.weapon.getWeight() + " Damage: " + i.weapon.getDamage()
                        + " Cost: " + i.weapon.getCost()
                        + "\n";
            }
        }
        return s;
